...
1. **InfinityFree** (`https://www.infinityfree.net`) — PHP + MySQL + cPanel
2. **000webhost** (`https://www.000webhost.com`) — PHP + MySQL + FTP
...
4. Valider email
5. Accueil InfinityFree s'affiche
...
1. Local : zipper le contenu du dossier `public/` :
```powershell
Compress-Archive -Path "C:\Users\LENOVO\LMS_Education\lms_platform\public\*" -DestinationPath "C:\Users\LENOVO\Desktop\public.zip"
```

2. Dans File Manager InfinityFree :
   - Entrer dans `public_html/`
   - Cliquer **"Upload"**
   - Sélectionner `public.zip`
   - Attendre upload
   - Clic droit sur ZIP → **"Extract"**
   - Supprimer le ZIP après extraction

3. Résultat : `public_html/` (tous fichiers du LMS dedans)

### A4. Créer base de données
1. Dashboard InfinityFree → **"Databases"**
...
1. Dashboard InfinityFree → **"phpMyAdmin"** (lien accès à la BD)
...
```php
<?php
define('DB_HOST', 'HOSTNAME_INFINITYFREE');  // Ex: sql123.infinityfree.com
...
```
4. Récupérer identifiants depuis l'email InfinityFree ou dashboard Databases
5. Cliquer **"Save"**

### A7. Tester accès
- Navigateur : `http://monlms.infinityfreeapp.com/`
...
Si vous voulez déployer plus rapidement et sans re-upload manuel, utilisez GitHub comme source et un workflow GitHub Actions pour envoyer automatiquement les fichiers à InfinityFree.
...
- Ouvrez `http://monlms.infinityfreeapp.com/`
- Si le site ne s’affiche pas, vérifiez :
  - `api/config.php` sur InfinityFree
...
- Si vous avez déployé le contenu dans `public_html/`, l’URL est :
  - `https://votresite.infinityfreeapp.com/`
- Si vous avez déployé dans `public_html/lms_platform/`, l’URL devient :
  - `https://votresite.infinityfreeapp.com/lms_platform/`
...
- Même processus que InfinityFree (télécharger ZIP, uploader, extraire)
...
2. Déployer public (InfinityFree/000webhost)
